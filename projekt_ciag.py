...
def symulacja_paliwa_stale(isp, m_paliwa, pc_design_bar, nazwa, n_exp, c_star, rho, a_si, gamma_spalin):
    ...
    # POPRAWKA: używamy np.trapezoid zamiast np.trapz dla numpy 2.0+
...
def symulacja_hybryda(isp_max, m_tot, pc_design_bar, nazwa, of_opt, rho, a_marxman, n_marxman, c_star_max, gamma_spalin):
    ...
    while Pc > P_ATM + 10 or m_ox_used < m_ox_tot:
        ...
        if mdot_f > 0:
            ...
            efektywnosc = np.clip(1.0 - 0.25 * np.abs(deviation), 0.5, 1.0)
        else:
            efektywnosc = 0.4
...
# ==============================================================================
# 3. BAZA DANYCH ORAZ GŁÓWNA PĘTLA
# ==============================================================================
DATA_PALIW = {
...
def uruchom_symulacje():
    ...
    for nazwa, d in DATA_PALIW.items():
        # Wywołanie symulacji
        if d["type"] == "solid":
            t, f, at_val, L_val, m_val, d_in_val, impuls = symulacja_paliwa_stale(d["isp"], d["m_paliwa"], d["pc_bar"], nazwa, d["n_exp"], d["C_star"], d["rho"], d["a_si"], d["gamma"])
        else:
            t, f, at_val, L_val, m_val, d_in_val, impuls = symulacja_hybryda(d["isp_max"], d["m_tot"], d["pc_bar"], nazwa, d["of_opt"], d["rho"], d["a_marxman"], d["n_marxman"], d["C_star_max"], d["gamma"])
        ...
        plt.plot(t, f, label=f"{nazwa} (d={d_sym_mm:.1f}mm)")

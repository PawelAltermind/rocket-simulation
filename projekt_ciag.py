...
def symulacja_paliwa_stale(isp, m_paliwa, pc_design_bar, nazwa, n_exp, c_star, rho, a_si, gamma_spalin):
    ...
    while Pc > P_ATM + 10 or (r_core < r_outer and L_g > 0):
        ...
        sliver_factor = 1.0 if web_burned < 0.94 * (r_outer - r_init) and L_g > 0.06 * L_grain else \
            np.clip((r_outer - r_core) / (0.06 * (r_outer - r_init)), 0, 1) * \
            np.clip(L_g / (0.06 * L_grain), 0, 1)
...
def symulacja_hybryda(isp_max, m_tot, pc_design_bar, nazwa, of_opt, rho, a_marxman, n_marxman, c_star_max, gamma_spalin):
    ...
    while Pc > P_ATM + 10 or m_ox_used < m_ox_tot:
        ...
        if mdot_f > 0:
            ...
            eff = np.clip(1.0 - 0.25 * abs(deviation), 0.5, 1.0)
        else:
            eff = 0.4

        c_star_inst = c_star_max * eff
        isp_inst = isp_max * eff
...
def uruchom_symulacje():
    ...
    for nazwa, d in DATA_PALIW.items():
        ...
        # combined plot
        plt.plot(t, f, label=nazwa)

        # per-engine figure
        plt.figure(figsize=(10,5))
        plt.plot(t, f)
        plt.title(f"Thrust profile - {nazwa}")
        plt.xlabel("Time [s]")
        plt.ylabel("Thrust [N]")
        plt.grid(True)
        plt.savefig(f"figures/{nazwa}_thrust.png", dpi=300, bbox_inches="tight")
        plt.close()
